...
- **Write tools** (20, annotated additive or destructive). **Additive** (16, `_WRITE_ADDITIVE_ANNOT`): `transition_card_lifecycle`, `create_risks`, `update_risks`, `add_card_comment`, `create_soaw`, `assign_stakeholders`, `update_cards_bulk`, `create_adr`, `update_adr`, `sign_adr`, `create_cards_bulk`, `create_diagram`, `import_bpmn`, `assign_card_tags`, `create_tag_group`, `create_tags`. **Destructive** (4, `_WRITE_DESTRUCTIVE_ANNOT`): `rollback_batch`, `update_diagram`, `archive_cards`, `upsert_relations_bulk`. **Tags** (`assign_card_tags` add/remove tags on cards, `create_tag_group`, `create_tags`) resolve tag/group NAMES to ids via `list_tag_groups` (a bare name must be unique across groups, else qualify as `'Group / Tag'`); they exist because tags are an M:N surface that `update_cards_bulk` can't touch (its `CardUpdate` schema silently drops a `tags` field — mirror of the `assign_stakeholders` precedent). The artifact-import subset (`create_cards_bulk` → `POST /cards/bulk-create`; `upsert_relations_bulk` → `POST /relations/bulk`; `create_diagram` → `POST /diagrams`; `import_bpmn` → find-or-create `BusinessProcess` then `PUT /bpm/processes/{id}/diagram`) lands structured rows the agent has read from its own context (spreadsheets, BPMN XML, DrawIO XML, PDFs, images). **Dry-run by default**: every mutating tool defaults to `dry_run=True`, which runs every validator and resolver server-side then rolls the transaction back so the agent can show the user a preview before committing. The backend's mutating endpoints carry the matching `dry_run: bool = False` request flag; `event_bus.publish` calls and side-effect emitters are gated on `not dry_run` so a preview never leaks events. Adding more mutating tools warrants careful security review.
